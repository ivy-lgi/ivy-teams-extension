...
This prototype showcases an app that sends a message to Teams with Adaptive Cards triggered by a HTTP post request. You can further extend the template to consume, transform and post events to individuals, chats or channels in Teams.

The prototype is built using the TeamsFx SDK, which provides a simple set of functions over the Microsoft Bot Framework to implement this scenario.
...
> **Prerequisites**
>
> To run the Ivy Teams Extension prototype in your local dev machine, you will need:
>
...
> **Note**
>
> The Ivy Teams Extension prototype can be installed as a personal app but could be extended to a team or a group chat. See [Installation and Uninstallation](https://aka.ms/teamsfx-notification-new#customize-installation).
...
5. Send a POST request to `http://<endpoint>/api/notification` with your favorite tool (like `Postman`) and the following body:
   ```
   {
      "receiver": "<The email address of your Microsoft Teams account>",
      "notification": {
         "kind": "new-task",
         "name": "Name",
...
| `src/adaptiveCards/new-task.json` | A generated Adaptive Card that is sent to Teams |
| `src/cardModels.ts` | The Adaptive Card data model for notifications ot the kind 'new-task' |
...
`src/adaptiveCards/new-task.json` defines the default Adaptive Card. You can use the [Adaptive Card Designer](https://adaptivecards.io/designer/) to help visually design your Adaptive Card UI.

`src/cardModels.ts` defines a data structure that is used to fill data for the Adaptive Card. The binding between the model and the Adaptive Card is done by name matching (for example,`NewTask.name` maps to `${name}` in the Adaptive Card). You can add, edit, or remove properties and their bindings to customize the Adaptive Card to your needs.
...
Congratulations, you've just created your own notification! To learn more about extending the Ivy Teams Extension prototype, [visit the documentation on Github](https://aka.ms/teamsfx-notification-new). You can find more scenarios like:
