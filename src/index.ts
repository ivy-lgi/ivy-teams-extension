...
import newTaskTemplate from "./adaptiveCards/new-task.json";
import { NewTask } from "./cardModels";
...
server.post(
  ...
  async (req, res) => {
    // This function will iterate all the installation points and send an Adaptive Card
    // to the installation of the user with the email address present as the value
    // of the attribute 'receiver' in the body of the request received.
    const member = await notificationApp.notification.findMember(
      async (m) => m.account.email === req.body.receiver
    );

    const notification = req.body.notification;

    if (notification.kind === "new-task") {
      await member?.sendAdaptiveCard(
        AdaptiveCards.declare<NewTask>(newTaskTemplate).render({
          kind: "New Task",
          name: notification.name,
          description: notification.description,
          start: new Date(notification.start).toLocaleString(),
          expiry: new Date(notification.expiry).toLocaleString(),
